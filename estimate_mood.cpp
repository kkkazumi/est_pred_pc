...
void get_para(int id_num, int set_num, int t, MatrixXd *W1_para);
...
int main(int argc, const char* argv[]){//(argv[0]=./estimate),argv[1]=usrname
  ...
  for(int id_num=0;id_num<9;id_num++){
    ...
    //for(int t_num=25;t_num<30;t_num++){
    int set_num = 10;
    ...
    int t =0;
    get_para(id_num,set_num,t,&W1_para);
    ...
    if(err<=0.0000001){
      errflg=1;
    }else if(count>=500000){//KENSYU: loop count
      errflg=1;
    }
    //}
    for(int i=0;i<N;i++){
...
void get_para(int id_num, int set_num, int t , MatrixXd *W1_para){
  ...
  sprintf(hap_in, "/home/kazumi/prog/est_pred_pc/data/%d/hap_weight.csv", id_num+1);//happyのパラメタを読み込む
  sprintf(sup_in, "/home/kazumi/prog/est_pred_pc/data/%d/sup_weight.csv", id_num+1);//supのパラメタを読み込む
  sprintf(ang_in, "/home/kazumi/prog/est_pred_pc/data/%d/ang_weight.csv", id_num+1);//angのパラメタを読み込む
  sprintf(sad_in, "/home/kazumi/prog/est_pred_pc/data/%d/sad_weight.csv", id_num+1);//sadのパラメタを読み込む
  ...
  for (int i = 0; i< 10; i++){
    ...
    sup_model_in >> ment_est_para[i][1];
    ang_model_in >> ment_est_para[i][2];
    sad_model_in >> ment_est_para[i][3];
